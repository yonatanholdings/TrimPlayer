...
- Some bugs may only occur on certain devices or versions of Android. Please add information about your device and the version of Android that is running on it (you can look these up under `Settings → About Phone`), as well as which version of TrimPlayer you are using.
- If the bug only seems to occur with a certain podcast, please include the URL of that podcast.
- If possible, add instructions on how to reproduce the bug.
- If possible, add a logfile to your post. This is especially useful if the bug makes the application crash. TrimPlayer has an `export logs` feature for this.
- Usually, you can take a screenshot of your smartphone by pressing *Power* + *Volume down* for a few seconds.
- Please use the following **[template](https://github.com/yonatanholdings/TrimPlayer/issues/new?assignees=&labels=Type%3A+Possible+bug&template=bug_report.yml)**.


How to submit a feature request
-------------------------------
- Make sure you are using the latest version of TrimPlayer. Perhaps the feature you are looking for has already been implemented.
...
- Please use the following **[template](https://github.com/yonatanholdings/TrimPlayer/issues/new?assignees=&labels=&template=feature_request.yml)**.


Submit a pull request
---------------------
- Before you work on the code
  - Make sure that there is an issue *without* the `Needs: Triage` or `Needs: Decision` label for the feature you want to implement or bug you want to fix. If you just start working on a feature that is not approved yet (or does not even have an issue), your PR might not get merged.
...
- Get coding :)
  ...
  - Please follow our code style. You can use Checkstyle within Android Studio using our [configuration file](https://github.com/yonatanholdings/TrimPlayer/blob/develop/config/checkstyle/checkstyle.xml).
  - To check the code style locally, run `./gradlew checkstyle spotbugsPlayDebug spotbugsDebug :app:lintPlayDebug`
  - Please only change the English string resources. Other languages are inherited from the upstream AntennaPod project.
- Open the PR
...
--------------------------
As a developer contributing to TrimPlayer, we ask that you test the feature yourself manually and, better yet, add unit and functional tests to any feature or bug you fix.
...
<img width="768" alt="trimplayer-run-tests"
